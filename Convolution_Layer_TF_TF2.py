...
class Convolution_Layer_TF(tf.keras.layers.Layer):
    
#%% Input-Independent Initialization(Layer Constructor Function)
    
    def __init__(self,
                 Tensor_Order,
                 Convolution_Subsection,
                 Filter_Size,
                 Number_of_Filters,
                 Convolution_Strides,
                 Weights_Initializer,
...
#-----------------------------------------------------------------------------#
        ...
        super(Convolution_Layer_TF,self).__init__(**kwargs)
        
#-----------------------------------------------------------------------------#
        
        self.Tensor_Order=Tensor_Order    # Set Tensor Order
        self.Convolution_Subsection=Convolution_Subsection    # Set Convolution Subsection
        self.Filter_Size=Filter_Size  # Set Filter Size
        self.Number_of_Filters=Number_of_Filters  # Set Number of Filters
        self.Convolution_Strides=Convolution_Strides  # Set Convolution Strides
...
#-----------------------------------------------------------------------------#
        ...
        # super(Convolution_Layer_TF,self).build(input_shape)  # Be sure to call this at the end
...
#-----------------------------------------------------------------------------#
        ...
        if self.Tensor_Order<=3:
            if self.Convolution_Subsection==2:
                ...
                # print('\nNot acceptable Convolution Subsection...\n')
                pass
...
#--------------Compute Tensor-based Convolution-Loop-Computation--------------#
...
# =============================================================================
            ...
            # print('\nComputing the Convolution between Tensors A and B-Loop-Computation...\n')
...
#-----------------------------------------------------------------------------#
            ...
            Frame_Input_Conv_3D=tf.unstack(inputs,
                                           axis=Stack_Axis)
            
            # Frame_Kernel_Conv_3D=tf.unstack(self.Weights,
            #                                 axis=Stack_Axis-1)
            Frame_Kernel_Conv_3D=tf.unstack(tf.reverse(self.Weights,
                                                       axis=tuple(list(range(self.Tensor_Order)))),
                                            axis=Stack_Axis-1)
...
#-----------------------------------------------------------------------------#
            ...
            Frame_Results=[]
            for i in range(Start_Index,Input_Frames,self.Convolution_Strides[-1]):
                Frame_Output_Conv_3D=[]
                ...
                if Padding_Mode=="VALID":
                    ...
                    if len(Indices_t_s)==Filter_Frames:
                        ...
                        for j,Index_t_i in enumerate(Indices_t_s):
                            Frame_Output_Conv_3D.append(tf.nn.convolution(input=Frame_Input_Conv_3D[Index_t_i],
                                                                          filters=Frame_Kernel_Conv_3D[j],
                                                                          strides=(1,)+self.Convolution_Strides[:-1]+(1,),
                                                                          padding=Padding_Mode))
                        Frame_Output_Conv_3D_Sum=tf.add_n(Frame_Output_Conv_3D)
                        Frame_Results.append(Frame_Output_Conv_3D_Sum)
                    ...
                    for Kernel_j,j in enumerate(Indices_t_s):
                        ...
                        if 0<=j<Input_Frames:
                            Frame_Output_Conv_3D.append(tf.nn.convolution(input=Frame_Input_Conv_3D[j],
                                                                          filters=Frame_Kernel_Conv_3D[Kernel_j],
                                                                          strides=(1,)+self.Convolution_Strides[:-1]+(1,),
                                                                          padding=Padding_Mode))
                    Frame_Output_Conv_3D_Sum=tf.add_n(Frame_Output_Conv_3D)
                    Frame_Results.append(Frame_Output_Conv_3D_Sum)
...
#-----------------------------------------------------------------------------#
            ...
            # print('\nThe Convolution between Tensors A and B-Loop Computation was computed successfully!\n')
...
#-----------------------------------------------------------------------------#
        ...
        config=super(Convolution_Layer_TF,self).get_config()
        
#-----------------------------------------------------------------------------#
        
        config.update({"Tensor_Order":self.Tensor_Order,
                       "Convolution_Subsection":self.Convolution_Subsection,
                       "Filter_Size":self.Filter_Size,
                       "Number_of_Filters":self.Number_of_Filters,
                       "Convolution_Strides":self.Convolution_Strides,
