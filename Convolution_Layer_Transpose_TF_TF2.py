...
#--------------------Convolution-Transpose-TF Layer Imports-------------------#
...
class Convolution_Layer_Transpose_TF(tf.keras.layers.Layer):
    
#%% Input-Independent Initialization(Layer Constructor Function)
    
    def __init__(self,
                 Tensor_Order,
                 Convolution_Transpose_Subsection,
                 Filter_Size,
                 Number_of_Filters,
                 Convolution_Transpose_Strides,
                 Weights_Initializer,
                 Bias_Initializer,
                 **kwargs):
...
#-----------------------------------------------------------------------------#
        ...
        super(Convolution_Layer_Transpose_TF,self).__init__(**kwargs)
        
#-----------------------------------------------------------------------------#
        
        self.Tensor_Order=Tensor_Order    # Set Tensor Order
        self.Convolution_Transpose_Subsection=Convolution_Transpose_Subsection    # Set Convolution Subsection
        self.Filter_Size=Filter_Size  # Set Filter Size
        self.Number_of_Filters=Number_of_Filters  # Set Number of Filters
        self.Convolution_Transpose_Strides=Convolution_Transpose_Strides  # Set Convolution Strides
...
#-----------------------------------------------------------------------------#
        ...
        # super(Convolution_Layer_Transpose_TF,self).build(input_shape)  # Be sure to call this at the end
...
#-----------------------------------------------------------------------------#
        ...
        if self.Tensor_Order<=3:
            if self.Convolution_Transpose_Subsection==2:
                Z=tf.nn.conv_transpose(input=inputs,
                                       filters=tf.reverse(self.Weights,
                                                          axis=tuple(list(range(self.Tensor_Order)))),
                                       strides=self.Convolution_Transpose_Strides,
                                       padding="SAME")
            elif self.Convolution_Transpose_Subsection==3:
                Z=tf.nn.conv_transpose(input=inputs,
                                       filters=tf.reverse(self.Weights,
                                                          axis=tuple(list(range(self.Tensor_Order)))),
                                       strides=self.Convolution_Transpose_Strides,
                ...
                # print('\nNot acceptable Convolution-Transpose Subsection...\n')
                pass
...
#---------Compute Tensor-based Convolution-Transpose-Loop-Computation---------#
...
# =============================================================================
            ...
            # print('\nComputing the Convolution-Transpose between Tensors A and B-Loop-Computation...\n')
...
#-----------------------------------------------------------------------------#
            ...
            for Index,Value in enumerate(Input_Shape[1:-1]):
                Output_Shape[Index+1]=Compute_Transpose_Convolution_Output_Length(dim_size=Input_Shape[Index+1],
                                                                                  stride_size=self.Convolution_Transpose_Strides[Index],
...
#-----------------------------------------------------------------------------#
            ...
            Frame_Input_Conv_Transpose_3D=tf.unstack(inputs,
                                                     axis=Stack_Axis)
            
            # Frame_Kernel_Conv_Transpose_3D=tf.unstack(self.Weights,
            #                                           axis=Stack_Axis-1)
            Frame_Kernel_Conv_Transpose_3D=tf.unstack(tf.reverse(self.Weights,
                                                                 axis=tuple(list(range(self.Tensor_Order)))),
                                                      axis=Stack_Axis-1)
...
#-----------------------------------------------------------------------------#
            ...
            for i in range(Start_Index,Input_Frames):
                Frame_Output_Conv_Transpose_3D=[]
...
# =============================================================================
#                             Frame_Output_Conv_Transpose_3D.append(tf.nn.conv_transpose(input=Frame_Input_Conv_Transpose_3D[Index_t_i],
#                                                                                        filters=Frame_Kernel_Conv_Transpose_3D[j],
#                                                                                        output_shape=Output_Shape[:-2]+(Output_Shape[-1],),
#                                                                                        strides=(1,)+self.Convolution_Transpose_Strides[:-1]+(1,),
...
# =============================================================================
                            Frame_Output_Conv_Transpose_3D.append(tf.nn.conv_transpose(input=Frame_Input_Conv_Transpose_3D[Index_t_i],
                                                                                       ...
                                                                                       strides=(1,)+self.Convolution_Transpose_Strides[:-1]+(1,),
                                                                                       padding=Padding_Mode))
                        Frame_Output_Conv_Transpose_3D_Sum=tf.add_n(Frame_Output_Conv_Transpose_3D)
                        
                        # Frame_Results.append(Frame_Output_Conv_Transpose_3D_Sum)
                        
                        Frame_Results.extend([Frame_Output_Conv_Transpose_3D_Sum
                                              for k in range(Compute_Transpose_Convolution_Output_Length(dim_size=1,
                                                                                                         stride_size=self.Convolution_Transpose_Strides[-1],
                    ...
                    for Kernel_j,j in enumerate(Indices_t_s):
                        ...
                        if 0<=j<Input_Frames:
# =============================================================================
#                             Frame_Output_Conv_Transpose_3D.append(tf.nn.conv_transpose(input=Frame_Input_Conv_Transpose_3D[j],
#                                                                                        filters=Frame_Kernel_Conv_Transpose_3D[Kernel_j],
#                                                                                        output_shape=Output_Shape[:-2]+(Output_Shape[-1],),
#                                                                                        strides=(1,)+self.Convolution_Transpose_Strides[:-1]+(1,),
#                                                                                        padding=Padding_Mode))
# =============================================================================
...
#-----------------------------------------------------------------------------#
            ...
            if Padding_Mode=="VALID":
                if len(Frame_Results)<Output_Shape[-2]:
                    ...
                    Frame_Results.extend([Frame_Output_Conv_Transpose_3D_Sum for l in range(Output_Shape[-2]-len(Frame_Results))])
...
#-----------------------------------------------------------------------------#
            ...
            # print('\nThe Convolution-Transpose between Tensors A and B-Loop Computation was computed successfully!\n')
...
#-----------------------------------------------------------------------------#
        ...
        config=super(Convolution_Layer_Transpose_TF,self).get_config()
        
#-----------------------------------------------------------------------------#
        
        config.update({"Tensor_Order":self.Tensor_Order,
                       "Convolution_Transpose_Subsection":self.Convolution_Transpose_Subsection,
                       "Filter_Size":self.Filter_Size,
                       "Number_of_Filters":self.Number_of_Filters,
                       "Convolution_Transpose_Strides":self.Convolution_Transpose_Strides,
